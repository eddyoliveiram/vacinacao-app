...
@section('content')
    ...
    <h2 class="text-2xl font-semibold mb-6">Adicionar Vacina</h2>
    ...
    <form method="POST" action="{{ route('vacinas.store') }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="lote" name="lote" value="{{ old('lote') }}" required
                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            @error('lote')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        ...
        <div class="mb-4">
            ...
            <input type="date" id="data_validade" name="data_validade" value="{{ old('data_validade') }}" required
                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            @error('data_validade')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
